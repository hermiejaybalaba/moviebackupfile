...
public class AvengersMovieController implements Initializable {
    ...
    @FXML
    private Button butTicketsButton;
    ...
    // Method to open the second FXML file and pass the selected date and time
 // Method to open the BuyTickets FXML file and pass the selected date and time
    @FXML
    private void openBuyTicketsFXML() {
        try {
            ...
            buyTicketsController.setMovieTitle("AVENGERS: END GAME");
            buyTicketsController.setMovieImage("@mov1.png"); 
    ...
    @FXML
    void onButTicketsClick2(ActionEvent event) {
        // You can add logic for the button click here, such as navigating to a new scene or showing a confirmation.
        // For example, navigating to the BuyTickets.fxml screen
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("BuyTickets.fxml"));
            Parent buyTicketsRoot = loader.load();

            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            Scene scene = new Scene(buyTicketsRoot);
            stage.setScene(scene);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
